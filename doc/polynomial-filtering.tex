...
The step function with $a = 0$ and $b = 1$ is know as the \emph{Heaviside
function}.
...
\begin{proof}
	...
	Using the facts that $\sin(0) = 0$, $\sin(\pi) = 0$, $\sin(\sfrac{1}{2} \pi)
	= +1$, and $\sin(\sfrac{3}{2} \pi) = -1$ \cite[§4.3.46]{AbramowitzStegun} as
	well as the fact that the sine function has periodicity $2\pi$
	\cite[§4.3.7]{AbramowitzStegun} completes the proof.
\end{proof}
...
be better. The following theorem shows that the use of Jackson coefficients
reduces the polynomial degree by one. In combination with the preceeding
Theorem~\ref{thm:chebyshev-heaviside-coefficients} and given a polynomial degree
$d$, the use of Jackson coefficients may reduce the polynomial degree by two
without affecting its convergence properties.
