...
assumes Hermitian positive-semidefinite matrices $A$. As far as we can tell,
Gaussian trial vectors cannot be used for the trace estimation of non-Hermitian
matrices because multiplying a vector with entries from the Gaussian
distribution with a unitary matrix gives another vector also with entries in
from the Gaussian distribution but this does not hold for non-unitary matrices.
Yet, this seems to be essential property in the proofs involving these trial
vectors. The Hutchinson estimator works with all matrices.

\begin{theorem}
	Let $A \in \F^{n,n}$, let $v \in \F^n$ have independently and identically
	distributed (i.i.d.) entries from the Rademacher distribution. Then
	\[ \mathbb{E}( v^* A v ) = \operatorname{tr}(A), \]
	where $\mathbb{E}(X)$ is the expected value of a randomly distributed
	variable $X$.
\end{theorem}
...
\begin{proof}
	...
	\]
	Since $v_i$ was drawn from the Rademacher distribution,
	\[ \mathbb{E}(v_i^2) = 1. \]
	Also, since all variables were drawn independently, the cases
	\begin{itemize}
		\item $v_i=+1$, $v_j=+1$, $v_i v_j = +1$,
		\item $v_i=+1$, $v_j=-1$, $v_i v_j = -1$,
		\item $v_i=-1$, $v_j=+1$, $v_i v_j = -1$, and
		\item $v_i=-1$, $v_j=-1$, $v_i v_j = +1$
	\end{itemize}
